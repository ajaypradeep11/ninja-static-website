...
Project categories use a sliding pill indicator in the desktop sidebar. On mobile, the **Filters** button expands the categories and selecting one closes them. Arrow Up/Down and Home/End navigate categories; Escape closes the mobile controls. The indicator does not animate when reduced motion is enabled.
