...
Project categories use a sliding pill indicator in the desktop sidebar. **Voice bots** is first and selected by default. Maple Clinic, Ember & Oak, Northshore Realty, Pulse, Rideau Motorworks, and Copper & Coil are sample projects: all six appear first under **UI/UX** and also under **Sample projects**. Cards retain the same business names across categories and use the relevant project-type tag plus a sample badge. Northshore Goods is a separate store-support sample, not Northshore Realty. On mobile, the **Filters** button expands the categories and selecting one closes them. Arrow Up/Down and Home/End navigate categories; Escape closes the mobile controls. The indicator does not animate when reduced motion is enabled.
